...
describe('dynamicHelpers', () => {
  ...
  describe('methods', () => {
    ...
    it('path for ‘en’ returns the locale url prefix ‘/’', () => {
      expect(
        getDynamicHelpers('en').path()('/de/test/test.html', mockRender)
      ).to.eq(`/${VERSION}/de/test/test.html`);
    });

    it('path for ‘de’ returns the locale url prefix ‘/de/’', () => {
      expect(
        getDynamicHelpers('de').path()('/test/test.html', mockRender)
      ).to.eq(`/${VERSION}/test/test.html`);
    });

    it('imagepath for ‘en’ returns the locale url prefix ‘/’', () => {
      expect(
        getDynamicHelpers('en').imagepath()('images/cat.gif', mockRender)
      ).to.eq(`/${VERSION}/images/cat.gif`);
    });

    it('imagepath for ‘de’ returns the locale url prefix ‘/de/’', () => {
      expect(
        getDynamicHelpers('de').imagepath()('images/cat.gif', mockRender)
      ).to.eq(`/${VERSION}/images/cat.gif`);
    });
    ...
    it('__ for ‘de’ returns the correct string “Datum/Uhrzeit des Builds”', () => {
      ...
      expect(getDynamicHelpers('de').__()('welcome.time', mockRender)).to.eq(
    ...
    it('__ for ‘en’ returns the correct string “Build date/time”', () => {
      // eslint-disable-next-line no-underscore-dangle
      expect(getDynamicHelpers('en').__()('welcome.time', mockRender)).to.eq(
        'Build date/time'
      );
    });
